...
class ServidorCerebro:
    ...
    def checar_todos_votos(self):
        if all(j['voto'] != '' for j in self.jogadores.values()):
            ...
            # [NOVO] Rotina separada de atualização de pontos para garantir entrega TCP
            for conn_jogador, dados in self.jogadores.items():
                try: 
                    conn_jogador.send(f"SCORE_UPDATE|PTS:{dados['pontos']}".encode('utf-8'))
                except: pass
